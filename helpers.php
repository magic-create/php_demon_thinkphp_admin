...
{
    return base_path() . 'admin' . ($path ? DIRECTORY_SEPARATOR . $path : $path);
}
...
{
    return app('admin.view')->make(mb_stripos($view, 'admin::') === false ? 'admin::' . $view : $view, $data, $mergeData)->render();
}
...
{
    return bomber()->strReplaceOnce(admin_url(), '', $url ? : request()->url(true), 'pre');
}
 ...
 * @copyright 魔网天创信息科技
 */
function admin_error($key, $replace = [], $locale = '')
{
    $name = "base.error.{$key}";
    $trans = app('admin.translation')->__($name, $replace, $locale);
