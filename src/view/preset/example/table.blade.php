        ...
        $(function(){
            ...
            $('body').on('toolbar:action', function(e, a){
                switch(a.action){
                    ...
                    //  新增用户
                    case 'add':
                        $.admin.table.form('新增用户', '{{request()->baseUrl(true)}}?_action=add', {
                    ...
                    case 'batch':
                        ...
                        $.admin.layer.confirm('确认将所选' + uids.length + '项进行' + a.$elem.html(), function(index){
                            $.post('{{request()->baseUrl(true)}}', {_action:a.$elem.data('action'), uid:uids, status:a.$elem.data('value')}, function(data){
            ...
            $('body').on('row:action', function(e, a){
                switch(a.action){
                    //  查看受邀人
                    case 'invited':
                        $.admin.table.layer('查看用户 : ' + a.row.nickname + ' 的受邀人', '{{request()->baseUrl(true)}}?_action=invited&inviteUid=' + a.row.uid, {full:true, btn:false});
                        break;
                    //  变更积分
                    case 'credit':
                        $.admin.api.open('变更用户 : ' + a.row.nickname + ' 的 ' + a.$elem.data('name'), '{{request()->baseUrl(true)}}?_action=credit&uid=' + a.row.uid + '&type=' + a.$elem.data('type'), function(index, layer){layer.iframe.$('form').submit();}, function(index, report){
                            if(report.status){
                                $.admin.layer.close(index);
                    ...
                    //  变更状态
                    case 'status':
                        $.post('{{request()->baseUrl(true)}}', {_action:'status', uid:a.row.uid, status:a.switch}, function(){$.admin.table.method('refresh');}).fail($.admin.api.fail);
                        break;
                    case 'edit':
                        $.admin.api.open('编辑用户 : ' + a.row.nickname, '{{request()->baseUrl(true)}}?_action=edit&uid=' + a.row.uid, function(index, layer){layer.iframe.$('form').submit();}, function(index, report){
                    ...
                    case 'del':
                        $.admin.layer.confirm('确认删除 : ' + a.row.nickname, function(index){
                            $.post('{{request()->baseUrl(true)}}', {_action:'delete', uid:a.row.uid}, function(data){
                        ...
                        break;
                    case 'get':
                        $.admin.api.open('查看用户 : ' + a.row.nickname, '{{request()->baseUrl(true)}}?_action=info&uid=' + a.row.uid, {success:function(layero, index){$.admin.layer.full(index);}}, function(index){$.admin.layer.close(index);});
                        break;
                    case 'invite':
                        $.admin.layer.confirm('确认模拟 : ' + a.row.nickname + ' 的邀请注册', function(index){
                            $.post('{{request()->baseUrl(true)}}', {_action:'invite', inviteCode:a.row.code}, function(data){
