...
use Demon\AdminThinkPHP\access\Service;
...
class DBTable
{
    ...
    public function getConfig()
    {
        ...
        //  设置URL
        $this->config->url = ($this->config->url ?? null) ? : request()->baseUrl(true);
        //  开启批量选择
    ...
    public function getQuery($step = 0)
    {
        ...
        if ($sortName) {
            ...
            $this->query->order($orderDynamic[0], $orderDynamic[1]);
        }
        //  附加排序
        foreach ($this->getOrder() as $key => $val) {
            //  如果是方法则使用DB::raw包裹
            if (bomber()->isFunction($val, false)) {
                $this->query->orderRaw(DB::raw(call_user_func($val, $key)));
                continue;
            }
            //  如果非字符串则直接使用
            else if (!is_string($val)) {
                $this->query->orderRaw(DB::raw($val));
                continue;
            }
            //  如果是括号包裹则使用DB::raw包裹
    ...
    public function template($type, $data = [], $content = null)
    {
        ...
        //  输出模板
        return app('admin')->view->make($this->template[$type] ? : config("dbtable.template.{$type}", ''))->with($data)->render();
    }
    ...
    {
        return $this->query->buildSql(false);
    }
    ...
    public function render($view, $data = [], $mergeData = [])
    {
        ...
        //  渲染视图
        return app('admin')->view->make(mb_stripos($view, 'admin::') === false ? 'admin::' . $view : $view, $data, $mergeData)->with('dbTable', $this);
    }
}
