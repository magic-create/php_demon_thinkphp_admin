...
namespace Demon\AdminThinkPHP\access;

use Demon\AdminThinkPHP\access\model\AllotModel;
...
class Service
{
    ...
    {
        $name = "base.access.{$key}";
        $trans = app('admin')->__($name, $replace, $locale);
    ...
    {
        return collect(AllotModel::where('uid', $uid)->get('rid'))->pluck('rid')->toArray();
    }
    ...
    {
        $self = RoleModel::order('system', 'desc');
        if ($filterStatus)
            $self->where('status', 1);
        $list = $self->get();
    ...
    {
        $mids = [];
        $list = collect(RoleModel::where('rid', $rids)->order('rid', 'desc')->get('mids'))->pluck('mids')->toArray();
        $all = collect(MenuModel::where('status', 1)->get('mid'))->pluck('mid')->toArray();
    ...
    {
        $mids = [];
        $list = collect(RoleModel::whereIn('rid', $rids)->where('status', 1)->order('rid', 'desc')->get('mids'))->pluck('mids')->toArray();
        $all = collect(MenuModel::where('status', 1)->get('mid'))->pluck('mid')->toArray();
        foreach ($list as $key => $val) {
            if ($val == '*') {
                if ($support)
...
EOF;
    ...
    public function getAccessAttribute($all, $origin, $badge)
    {
        ...
        foreach ($data as &$item) {
            ...
            //  当前
            $item['active'] = $item['type'] == 'page' && $item['path'] == request()->baseUrl(true) ? 'mm-active' : '';
            //  父类
    ...
    public function getBreadcrumb($isDetails = false)
    {
        if (!$this->breadcrumb) {
            $url = '';
            foreach (explode('/', ltrim(request()->baseUrl(), '/')) as $index => $title) {
                $url .= '/' . $title;
    ...
    public function intercept(string $action, array $except = [], array $assgin = [])
    {
        ...
        foreach ($assgin as $parent => $childs) {
            ...
            if (in_array('*', $childs) || in_array($action, $childs)) {
                ...
                //  如果父指派通过则通过
                if (self::check(bomber()->strReplaceOnce($needle, $replace, request()->baseUrl(true), 'post')))
                    return true;
            }
        }

        //  在规则中通过检查
        return self::check(request()->baseUrl(true));
    }
